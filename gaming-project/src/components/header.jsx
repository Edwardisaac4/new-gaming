...
const Header = () => {
  ...
  return (
    <header
      className={`py-1 px-7 flex justify-between items-center fixed z-50 w-full transition-all duration-300 ${isScrolled
          ? "bg-[#0a0a0a]/80 backdrop-blur-lg border-b-[0.4px] border-[#babaff]"
          : "bg-transparent border-b-[0.4px] border-transparent"
        }`}
    >
      {/* Desktop Menu */}
      ...
      <div
        id="mobileMenu"
        className={`fixed md:hidden top-14 right-0 left-0 bg-[#0a0a0a]/80 backdrop-blur-lg h-screen p-5 transition-transform duration-300 ease-in-out transform ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        ...
        <div className="flex flex-col gap-4 mt-6">
          <button className="bg-blue-600 rounded py-3 mt-4 cursor-pointer hover:bg-green-400">
            Play Now
          </button>

          <button className="bg-purple-600 rounded py-3 cursor-pointer hover:bg-amber-400">
            NFT Store
          </button>
        </div>
      </div>
